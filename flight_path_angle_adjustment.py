...
class AngleVary():
	...
	def __init__(self, aero_coeff, velocity, start_angle):
		# Creates empty data list which will store all trim conditions 
		self.data = []

		# Works out initial trim and adds this to data list
		self.initial_trim = Trim_Conditions(aero_coeff, velocity, start_angle)
		...
		self.angle_adjustment(aero_coeff, velocity, start_angle, 0.01)
		...
		self.angle_vary1, self.angle_vary2 = graph_creator.plot_generator(self, vel_vary=False)
		

		

	def angle_adjustment(self, aero_coeff, velocity, start_angle, direction):
		flight_path_angle = start_angle
		...
		while self.constraint:
			...
			flight_path_angle = flight_path_angle + direction
	...
	def constraint_check(self): 
		...
		and self.all_thrusts[-1] >0 and self.all_h_vel[-1]>0 and self.all_v_vel[-1]>0): 
			self.constraint = True 
		else: 
